...
const generateUser = (index, tribes) => ({
  ...
  get username() { return this.displayName.toLowerCase().replace(/'/g, '').replace(/\s/g, ''); },
  memberIds: selectRandom(tribes, 0.4).map(tribe => tribe._id),
});

const generateTribes = count => [...Array(count).keys()].map(i => generateTribe(i));
const generateUsers = (count, tribes) => [...Array(count).keys()].map(i => generateUser(i, tribes));
